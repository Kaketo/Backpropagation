...
class Network():
    ...
    def train_once(self, x, y, x_test, y_test):
        ...
        # Return errors
        return [train_error, test_error, self.return_weights()]

    def train_and_evaluate(self, x, y, x_test, y_test, show_epoch = True):
        """
        ...
        weights_norms = [None] * (self.iterations)
        # Network training and errors calculation
        for i in range(self.iterations):
            if show_epoch:
                print('\rEpoch: ' + str(i + 1) + '/' + str(self.iterations), end = '')
            [training_set_error[i], test_set_error[i], weights_norms[i]] = self.train_once(x, y, x_test, y_test)
        return [training_set_error, test_set_error, weights_norms]

    def train_mini_batch_and_evaluate(self, x, y, x_test, y_test, mini_batch_size, show_epoch = True):
        """
        ...
        for i in range(self.iterations):
            ...
            for j in range(len(mini_batches_data)):
                [training_set_error[i], test_set_error[i], weights_norms[i]] = self.train_once(np.array(mini_batches_data[j]), np.array(mini_batches_answer[j]), x_test, y_test)
        return [training_set_error, test_set_error, weights_norms]
